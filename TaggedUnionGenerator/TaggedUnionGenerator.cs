...
namespace TaggedUnionGenerator
{
    ...
    public class TaggedUnionGenerator : IIncrementalGenerator
    {
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            {
                var (isValid, validatedDefinition) = ValidateOptions(ctx, def);
                if (!isValid)
                {
                    return;
                }
                ...
                try
                {
                    ...
                    UnionTypeWriter.Write(validatedDefinition, sourceWriter);
        ...
        private static (bool, UnionTypeDefinition) ValidateOptions(SourceProductionContext ctx, UnionTypeDefinition def)
        {
            var validationPassed = true;

            var optionNameUniqueCheck = new HashSet<string>();
            var optionTypeUniqueCheck = new HashSet<string>();
            ...
            foreach (var op in def.Options)
            {
                ...
                else if (!optionNameUniqueCheck.Add(op.Name))
                {
                ...
                if (!optionTypeUniqueCheck.Add(op.Type))
                {
                    var diag = Diagnostic.Create(Diagnostics.WarningCastOperatorsDisabled, op.Location);

                    ctx.ReportDiagnostic(diag);

                    def = def with { GenerateCastOperators = false };
                }
            }

            return (validationPassed, def);
        }
